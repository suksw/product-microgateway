 ...
 */
public class WebsocketApiThrottleServerTestCase extends ApimBaseTest {
    ...
    public void testAPIServerBandwidthThrottling() throws Exception {
        ...
        Assert.assertTrue(wsClient.isServerBandwidthThrottled(throttleBandwidth),
                "Websocket connection was not throttled on bandwidth.");
    }

    @Test(description = "Test API Level EventCount Throttling")
    public void testAPIServerEventCountThrottling() throws Exception {
        ...
        Assert.assertEquals(updatedAPI.getApiThrottlingPolicy(), EVENT_COUNT_THROTTLE_API_POLICY_NAME,
                "API tier not updated.");

        // create Revision and Deploy to Gateway
        PublisherUtils.createAPIRevisionAndDeploy(eventCountThrottleApiId, publisherRestClient);
        Utils.delay(TestConstant.DEPLOYMENT_WAIT_TIME*2,
                "Couldn't wait until the API was deployed in Choreo Connect");

        WsClient wsClient = new WsClient(eventCountThrottleEndpoint, headers);
        Assert.assertTrue(wsClient.isServerEventCountThrottled(throttleCount),
                "Request not throttled by event count");
    }
}
